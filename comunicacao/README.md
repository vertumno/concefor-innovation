# VIII Concefor 2026 — Central de Comunicação da CGTE

> **Área de comunicação** do repo único do Concefor (`concefor-innovation`), da Coordenadoria
> Geral de Tecnologias Educacionais (CGTE): planejar, produzir e distribuir as peças de
> comunicação do VIII Concefor. Era o repo `concefor2026`, incorporado aqui em 16/07/2026
> com histórico git preservado.
>
> **Tema:** *"Tecnologia, Transformação e Educação a Distância: 20 anos construindo novos cenários educacionais"*
> **Data do evento:** 17, 18, 19 e 20 de agosto de 2026 · Cefor / Ifes · Vitória (ES)
> **Marco:** Celebração dos **20 anos do Cefor** (selo comemorativo em todas as peças)

---

## Como esta pasta funciona

Esta pasta é organizada por **engenharia de contexto**: separa o que o time *sabe* (contexto)
do que o time *faz* (produção) e de como o time *se organiza* (planejamento). Qualquer pessoa —
ou IA — que precise agir deve conseguir carregar o contexto e entender o estado de tudo.

```
comunicacao/
├── _inbox/            ← Jogue aqui tudo que surgir. Depois processamos e distribuímos.
├── contexto/          ← O CÉREBRO. Fatos canônicos do evento (fonte de verdade).
├── producao/          ← As peças sendo produzidas, por fase (brindes, pré, durante, ideias).
├── planejamento/      ← Board de produção, calendário de publicação, distribuição da equipe.
├── templates/         ← Modelos de brief e roteiro para iniciar uma peça nova.
├── site-concefor.md   ← Cópia bruta do conteúdo do site oficial (fonte primária).
└── README.md          ← Você está aqui.
```

### O fluxo do `_inbox`
1. Surgiu qualquer coisa (ideia, arquivo, link, pedido)? → jogue em [_inbox/](_inbox/)
2. Em algum momento processamos o inbox: cada item vira uma peça em `producao/`,
   um fato em `contexto/`, ou uma tarefa no board de `planejamento/`.
3. Item processado sai do inbox.

---

## Atalhos rápidos

| Preciso de… | Vá para |
|---|---|
| Fatos do evento (datas, tema, local) | [contexto/00-evento.md](contexto/00-evento.md) |
| Programação dia a dia | [contexto/01-programacao.md](contexto/01-programacao.md) |
| Datas e prazos (cronograma) | [contexto/02-cronograma.md](contexto/02-cronograma.md) |
| Palestrantes (bios, fotos, status) | [contexto/03-palestrantes.md](contexto/03-palestrantes.md) |
| Quem faz o quê na CGTE | [contexto/04-equipe-cgte.md](contexto/04-equipe-cgte.md) |
| Identidade visual, selo 20 anos, Canva | [contexto/05-identidade-visual.md](contexto/05-identidade-visual.md) |
| Canais, tom de voz e diretrizes | [contexto/06-canais-e-diretrizes.md](contexto/06-canais-e-diretrizes.md) |
| Eventos paralelos (**6 confirmados, todos com local**) | [contexto/07-eventos-paralelos.md](contexto/07-eventos-paralelos.md) |
| Escrever notícia para o site (estilo + exemplos) | [contexto/08-referencia-noticias.md](contexto/08-referencia-noticias.md) |
| **Estado de TODAS as peças** | [planejamento/board-producao.md](planejamento/board-producao.md) |
| ✅ **Checklist da reta final** (o que falta, por prazo, até 17/08) | [planejamento/checklist-reta-final.md](planejamento/checklist-reta-final.md) |
| **Painel visual do status** (abrir no celular / mostrar em reunião) | [painel/](painel/) · [no ar](https://vertumno.github.io/concefor-innovation/comunicacao/painel/) |
| **Kit dos eventos paralelos** (mandar para as 6 coordenações: copiar texto e baixar card) | [kit-eventos-paralelos/](kit-eventos-paralelos/) · [no ar](https://vertumno.github.io/concefor-innovation/comunicacao/kit-eventos-paralelos/) |
| Calendário de posts | [planejamento/calendario-publicacao.md](planejamento/calendario-publicacao.md) |
| Escalas (podcast, transmissão, fotografia) | [planejamento/distribuicao-equipe.md](planejamento/distribuicao-equipe.md) |
| **Planilha de atuação da CGTE no evento** (uma aba por dia) — **oficial e viva, sempre ler o Drive** | [📊 Planilha no Drive (canônica)](https://bit.ly/conceforcgte) · [cópia local](planejamento/ATUACAO%20DA%20CGTE%20NO%20CONCEFOR%202026.xlsx) · [modelo de 2024](planejamento/_referencias/ATUACAO%20DA%20CGTE%20NO%20CONCEFOR%202024.xlsx) |
| Começar uma peça nova | [templates/brief-peca.md](templates/brief-peca.md) |
| **Pedidos da Márcia** (atualização do site + novas peças) | [Doc de solicitações (Google Docs)](https://docs.google.com/document/d/1un58VaMY0wwIVaxfcf9Ssb3tBKuKz_Nj/edit?usp=sharing&ouid=102948503336580466969&rtpof=true&sd=true) |
| Artes produzidas (Canva) | [Pasta no Canva](https://canva.link/wdnsx8ggf61x2hs) |
| Matrizes do Illustrator | [Drive da CGTE](https://drive.google.com/drive/folders/1fuoepIyukqXTw035JtTj-MSbnpw7veJn?usp=drive_link) |
| Editáveis públicos (compartilhar com a comissão) | [Drive](https://drive.google.com/drive/folders/1-CH92C467aI7C_dj6fE4O5XBI8A_qShv) |

---

## Estado do momento (atualizar sempre)

- **Hoje:** 13/08/2026 (quinta) — faltam **4 dias** para a abertura (17/08).
  📋 **O que falta, item a item e por prazo:**
  [checklist da reta final](planejamento/checklist-reta-final.md).
  📊 **A planilha suprema da operação:** https://bit.ly/conceforcgte
- 🗓️ **A semana decisiva.** Três datas mandam em tudo daqui pra frente:
  **14/08 (sexta)** a gráfica entrega **todo o material impresso de uma vez** (brindes + banners
  do TR + os institucionais) · **15/08 (sábado)** fecham as inscrições ·
  **17/08 (segunda)** abre o evento, e às 20h é o lançamento do e-book do NTE.
- ✅ **A reunião de 12/08 tirou cinco frentes da mesa da CGTE** (detalhes em
  [`decisoes.md`](../decisoes.md)): **etiquetas de nome** (comissão geral assumiu) ·
  **link do livro dos 20 anos** (definitivo) · **banners institucionais** (na gráfica) ·
  **cantinho instagramável** (CGA) · **vídeos-convite** (cancelados). Sobram três coisas nossas:
  **artes dos 2 cards de mesa**, **contagem regressiva** e **3 hosts de podcast**
  *(eram 2 quando os episódios eram 4; a grade de 12/08 passou para 8)*.
- ⚠️ **O que a entrega em 14/08 ainda significa:** o prazo contratado era 07/08 e escorregou uma
  semana. Com as etiquetas fora da nossa mesa, o que resta é **conferir as peças impressas** assim
  que chegarem — erro de horário, nome ou sala em banner não tem conserto a três dias do evento.
  **Elton faz a conferência.**
- **Cadência de publicação (definida em 27/07):** **um card por dia, em sequência, e só cards.**
  Os stories programados foram retirados do [calendário](planejamento/calendario-publicacao.md).
- **Já no ar:** card do prazo de envio (16/07) · [Vanessa Battestin](https://www.instagram.com/p/DbF30gOOToU/) (21/07) ·
  [Felipe Tessarolo](https://www.instagram.com/p/DbF4cUPh-TT/) (24/07) ·
  [Mariano Pimentel](https://www.instagram.com/p/DbT3A9apgf0/) (27/07) ·
  [Rutinelli Fávero](https://www.instagram.com/p/DbT4FRUJKh1/) (28/07) ·
  [Márcia Oliveira](https://www.instagram.com/ifescefor/p/DbT5NHkJCqP/) (29/07) ·
  [Mariella Berger](https://www.instagram.com/ifescefor/p/DbT43IkpyLd/) (30/07) ·
  [Jaqueline Sanz](https://www.instagram.com/ifescefor/p/DbT4ejZpGSv/) (31/07).
  ✅ **A série dos 7 individuais está fechada.**
- ✅ **Card do Mauro Oliveira publicado em 03/08** ([post](https://www.instagram.com/ifescefor/p/DblPpaYuAN9/)), **antecipado de 11/08** para o
  feed não passar mais um dia vazio. **Com ele, os 8 cards de palestrante estão no ar.**
  ⚠️ Foi ao ar com a **v1, que diz "MESA"** onde a atividade é **palestra**; a legenda publicada
  está correta.
- ✅ **11/08 — o feed voltou.** O **carrossel dos eventos paralelos** foi publicado
  ([post](https://www.instagram.com/p/Db6dlZPm7kS/?img_index=1)), encerrando os 8 dias de silêncio
  desde o card do Mauro (03/08).
- 🟡 **Os dois cards de mesa seguem sem arte** ("Desafios da EaD", que era 01/08, e
  "Tecnologia Delas", 02/08). **Em aberto na reunião de 12/08 — "ainda vamos ver".**
  ✳️ **O que já está pronto para quando a decisão sair:** existe no repo um **pipeline de cards
  por código** (HTML/CSS → Chrome headless → PNG 1080×1350), o mesmo que gerou os 6 cards dos
  eventos paralelos, na família visual dos cards de palestrante. Para as duas mesas já estão em
  mãos as **legendas com alt-text** ([8 e 9 em `legendas.md`](producao/02-pre-evento/palestrantes/legendas.md))
  e as **5 fotos** ([`_refs/fotos/`](producao/02-pre-evento/palestrantes/_refs/fotos/)): Rutinelli
  e Tessarolo numa; Márcia, Mariella e Jaqueline na outra. **Não depende de designer nem de Canva.**
- **Inscritos:** **288** pela API do Even3 (16/07) — capacidade 400. *(número de 16/07; vale
  reconsultar na semana do evento, com as inscrições fechando em 15/08.)*
- **Próximos prazos externos:** ✅ envio final dos trabalhos encerrado em **03/08 (Even3)** ·
  🔴 **inscrições até 15/08 (sábado)** · 🔴 **gráfica entrega tudo em 14/08 (sexta)**.
- **Gráfica:** artes enviadas em 22/07. ✅ **Amostras dos brindes avaliadas e finalizadas em 28/07**
  — conduzido por **Viviane e Dennis**.
  ✅ **05/08 — as artes dos banners chegaram da Juliana e estão no repo**
  ([`banners/`](producao/01-brindes-promocionais/banners/)): PDFs de #01 a #06 e os quatro PNGs de
  localização. ✅ **Seguiram para a gráfica.**
  🔴 **A entrega mudou: era 07/08, agora é 14/08 (sexta), e vem tudo de uma vez** — brindes e
  banners do TR no mesmo lote, e agora **também os 3 institucionais**, cuja arte foi enviada e
  fica pronta até sexta. É o que aperta a semana da abertura.
  ✅ **02/08 — Caneta, copo bucks e bolsa APROVADOS** — todas as montagens da gráfica ok.
  👕 **Grade final das camisas, enviada à gráfica em 29/07:**
  **PP 2 · P 20 · M 30 · G 28 · GG 16 · XG 2 · XXG 2 = 100** — substitui a de 28/07
  (25 P · 30 M · 30 G · 15 GG), que não tinha PP, XG nem XXG.
  [E-mail](producao/01-brindes-promocionais/_especificacoes/email-grafica-envio-artes.md)
  ℹ️ *(histórico)* quem conduz o contato com a gráfica esteve **de férias de 27 a 31/07**, janela em
  que o acompanhamento ficou com o **Marquito** — já encerrada.
  ⚠️ A gráfica manda as artes **por WhatsApp**; pedimos em 28/07 que envie **também por
  e-mail**, senão a definição não fica rastreável. Vale reforçar na entrega de 14/08.
- ✅ **Nomes nos crachás: FORA DA CGTE (12/08).** Os crachás vêm todos em branco, e a **comissão
  geral assumiu e já está providenciando a impressão dos nomes**. Encerra a frente aberta em 27/07
  e o risco de véspera que era a prioridade nº 2 da operação. O histórico da apuração (área livre
  de 100 × 43,7 mm medida em 03/08, Pimaco A4360 recomendada) fica registrado em
  [`etiquetas-nomes.md`](producao/01-brindes-promocionais/cracha/etiquetas-nomes.md) caso a
  comissão precise da medida.
- **Transmissão + Libras:** ✅ definido — **todas as palestras e mesas-redondas** (16/07).
- **Podcasts (12/08): os 8 palestrantes do evento têm episódio, em 7 gravações** — Jaqueline e
  Mariella gravam **juntas**, às 11h30 de 18/08, direto da mesa "Tecnologia Delas".
  ✅ **Grade fechada:** cada gravação é um **bloco de 40 min no mesmo dia da fala** do convidado,
  o que tirou a dependência de alguém ficar até quinta. 6 das 7 são **depois** da fala; só a
  Vanessa grava antes, porque a noite de abertura não deixa janela. ✅ **7 roteiros escritos.**
  ✅ **Mariana é host da dupla de IA**; 🔴 **faltam 3 hosts** (agrupados por conjunto temático) e
  🔴 **falta a sala de gravação** — a terça vai das 11h30 às 17h50 e o set precisa comportar duas
  convidadas. [Plano](producao/03-durante-evento/podcasts/plano-podcasts.md) ·
  [roteiros](producao/03-durante-evento/podcasts/roteiros/)
- **Aguardando da Márcia/Coordenação:** ✅ camisa (**opção 2**) e bloco **aprovados em 28/07** ·
  🟡 **3 hosts de podcast** (a Mariana já assumiu o conjunto de IA; são 7 episódios em 4 conjuntos) ·
  ✅ eventos paralelos completos (29/07) · logo do Educimat p/ impressos · revisão da
  [notícia para a ACS](producao/02-pre-evento/noticia-acs/rascunho-noticia.md) ·
  atualização do **Even3** para espelhar o site (o app do evento lê de lá).
- 📋 **Reunião de escala, 12/08 (78 min):** a planilha de atuação saiu do esqueleto e ganhou nomes
  hora a hora nos quatro dias.
  [Síntese completa](../contexto/reunioes/sintese-2026-08-12.md) ·
  [planilha canônica](https://bit.ly/conceforcgte).
  **Responsáveis fixos:** postagens **Elton** · app admin **Marquito** · termos do podcast
  **Andreia** · transmissão **Leo e Tiago** · cobertura e fotografia **Nanda, Andreia e Juliana**.
  ✅ **Confirmado que reels e stories são da CGTE**, no Instagram do Cefor, igual a 2024.
  🔴 **Limite declarado:** a CGTE **não fotografa apresentação por apresentação** nas sessões
  técnicas e na mostra — a cobertura é geral. Marquito confirma isso com a Márcia.
  ⚠️ **Marquito e Elton dão oficina na quinta inteira** e ficam fora da cobertura no dia dos 6
  eventos paralelos.
- 🆕 **Cinco demandas apareceram nessa reunião, todas sem dono:** **thumbnails das 8
  transmissões** · **modelo único de slides** (palestrantes, sessões técnicas, lançamentos,
  premiação) · **reações do app sobre a transmissão** · **espaço compartilhado** para o material
  gravado em celular · **aba de links úteis** na planilha.
- 🔴 **Dois erros achados ao conferir a planilha:** o **link da transmissão do Mariano é o mesmo do
  Tessarolo** (`2fCU8tmmXoo`) — quem divulgar manda o público para a live do dia anterior; e a
  **Eliana está nos 7 momentos de Libras sem uma folga**.
- 🔴 **O app entrou em risco maior que a divulgação: os servidores do Ifes estão fora do ar.** Não
  dá para mexer no app e não se sabe se ele sobe na segunda. Precisa de plano B declarado.
- **Prioridades agora (recalculadas na reunião de 12/08, por prazo e por risco):**
  0. 🖼️ **Thumbnails das 8 transmissões — a primeira live é segunda.** Sem capa, a live entra no
     feed e no compartilhamento como retângulo cinza. Sai pelo mesmo pipeline de código dos cards,
     em 1280×720; as 8 numa rodada só.
  1. 🔴 **Hospedagem do e-book do NTE — 5 dias, e é lançamento de palco.** Em **17/08 às 20h** o
     e-book é anunciado e ainda não tem endereço de acesso: sem local não há link nem QR code.
     Recomendação no brief: publicar na **Base de Conhecimentos**, que já está no ar e mantém o
     link válido depois do evento. ✅ **O livro dos 20 anos já se resolveu** (link definitivo
     abaixo) — **o do NTE é o único que sobrou**. O **card** que a Vanessa pediu depende disso e
     da **sinopse do NTE**.
     [Brief + mensagem pronta para a Vanessa e a Andrômeda](producao/02-pre-evento/livro-nte/brief.md)
  2. 🔴 **Contagem regressiva — decidido em 12/08 que VAI SER FEITA, e a janela é agora.**
     Restam 15, 16 e 17/08. Como a regra é só cards e 9 das 10 peças eram stories, o que sobra é
     converter para card de feed: **"faltam 2 dias" + último dia de inscrição (15/08)**,
     **"é amanhã" (16/08)** e **"é hoje" (17/08)**.
     [Calendário](planejamento/calendario-publicacao.md)
  3. 🟡 **Artes dos 2 cards de mesa — em aberto ("ainda vamos ver").** Tudo pronto menos a
     decisão: legenda, alt-text, fotos e pipeline. Se sair hoje ou amanhã, ainda cabem antes da
     regressiva; depois de 14/08 elas disputam espaço com ela no feed.
  4. 🟡 **Faltam 3 hosts de podcast.** Com a grade de 12/08 (**os 8 palestrantes, em 7
     gravações**), os episódios foram agrupados em 4 conjuntos temáticos, então 4 pessoas cobrem
     tudo. A **Mariana** assume o conjunto de IA (Mauro + Mariano); faltam "Tecnologia Delas" (2
     episódios), "Desafios da EaD" (2) e "20 anos" (1) — os dois últimos podem ser a mesma pessoa.
     Junto disso, falta a **sala de gravação**, e a terça precisa dela o dia inteiro.
     [Plano](producao/03-durante-evento/podcasts/plano-podcasts.md)
  5. ✅ **Eventos paralelos — ENVIADO E PUBLICADO (11/08).** As 6 coordenações receberam card e
     texto pelo [kit](https://vertumno.github.io/concefor-innovation/comunicacao/kit-eventos-paralelos/),
     e o carrossel foi ao ar no Instagram
     ([post](https://www.instagram.com/p/Db6dlZPm7kS/?img_index=1)). Os eventos são **20/08** e
     quem divulga são elas, não nós.
  6. ❌ **Vídeos-convite dos palestrantes — CANCELADOS (12/08).** Não serão feitos, e com eles
     caem os reels de 08 a 14/08 que dependiam do material.
  7. 📸 **Cantinho instagramável — FORA DA CGTE (12/08).** **Luciane Lacerda e Carol, da CGA**,
     são as responsáveis. Confirma a posição que a Juliana defendeu em 03/08.
  8. ❓ **Divulgação do app aos inscritos — travada por problema nos sistemas do Ifes.** O e-mail
     aos 288 inscritos não saiu e a causa é externa à CGTE. Vale ter plano B: QR no crachá, fala
     de 1 min na abertura e o QR do telão ocioso (`/projecao`).
  9. ✅ **Os 10 banners do evento — artes entregues e na gráfica.** A Juliana entregou em **05/08**
     e os arquivos estão no repo ([`banners/`](producao/01-brindes-promocionais/banners/)): PDFs de
     #01 a #06 e **quatro PNGs de localização**. Impressos voltam em **14/08**.
     ✳️ **Duas dúvidas antigas foram respondidas pela própria arte:** os banners de andar **não são
     4 iguais** — cada um marca "VOCÊ ESTÁ AQUI" no seu piso (Térreo, 1º, 2º, 4º) — e o **3º piso
     ficou fora**, por não ter atividade. Saíram sem logo do Concefor e sem selo, como decidido.
     ⚠️ **Conferência quando chegarem: Elton** (definido em 12/08). A Juliana pediu formalmente essa
     revisão em 03/08 e ela não tinha dono; erro de horário, nome ou sala só aparece com a peça
     impressa em mãos, em 14/08.
     [Brief, com os ajustes banner a banner](producao/01-brindes-promocionais/banners/brief.md)
  10. ✅ **Banners institucionais — RESOLVIDOS (12/08): artes enviadas à gráfica, prontos até
      sexta (14/08).** Chegam junto com o lote do TR.
      🔗 **Os três links fecharam:** Base de Conhecimentos (29/07) · MOOCs
      https://mooc.cefor.ifes.edu.br/ (10/08) · **livro dos 20 anos (12/08)**,
      https://cefor.ifes.edu.br/index.php/publicacoes/2-uncategorised/17710-livro20anos — endereço
      no site institucional, não mais o Drive provisório, o que era condição para o QR impresso.
      [Brief](producao/02-pre-evento/banners-institucionais/brief.md)
  11. ✅ ~~Conferir a 3ª leva de artes da gráfica~~ — **caneta, copo e bolsa aprovados em 02/08**.
  12. ✅ ~~**Card do Mauro Oliveira**~~ — **publicado em 03/08**, antecipado de 11/08.
- **Canva (produção de artes):** https://www.canva.com/folder/FAHDrZ44PIM

> Mantenha este bloco e o [board de produção](planejamento/board-producao.md) atualizados —
> eles são o "status ao vivo" da operação.
