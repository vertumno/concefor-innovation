# Checklist da reta final — 13 a 17/08/2026

> Tudo que está pendente na comunicação, organizado **por prazo**. Gerado em **13/08 (quinta)** a
> partir do [board](board-producao.md), da
> [síntese da reunião de 12/08](../../contexto/reunioes/sintese-2026-08-12.md), da
> [planilha de atuação](https://bit.ly/conceforcgte)
> e do estado real do repositório.
>
> 🔴 crítico · 🟡 importante · 🔵 desejável · **⚠️ sem dono**

**Faltam 4 dias para a abertura.** Amanhã (14/08, sexta) chega todo o material impresso; sábado
15/08 fecham as inscrições; segunda 17/08 abre o evento, e às 20h é o lançamento do e-book do NTE.

---

## 🔥 Hoje (13/08, quinta)

- [ ] 🔴 **Resolver a hospedagem do e-book do NTE.** É a **única trava** que sobrou dessa frente:
      o PDF oficial já está no repo e a notícia do site já está escrita, mas o CTA e o QR apontam
      para `LINK-A-DEFINIR`. Sem link, **nem a notícia nem o card podem ir ao ar**, e o lançamento
      é segunda às 20h. Caminho já validado: a mesma página de publicações do site do Cefor que
      resolveu o livro dos 20 anos. → **Elton leva à Vanessa / Andrômeda**
- [ ] 🔴 **Fechar a grade dos podcasts** — a reunião de ontem marcou hoje para isso. Definir
      horários, falar com cada convidado e **reconciliar a planilha (5 episódios + repescagem) com
      o [`plano-podcasts.md`](../producao/03-durante-evento/podcasts/plano-podcasts.md) (7
      episódios)**. A versão escolhida volta para a planilha, que é o que vai impresso.
      → **Marquito / Elton**
- [ ] 🔴 **Definir os hosts que faltam** — a Mariana assume dois; o Marquito se ofereceu para
      outros. → **Coordenação**
- [ ] 🔴 **Corrigir o link duplicado da transmissão do Mariano** — hoje aponta para
      `youtube.com/live/2fCU8tmmXoo`, o mesmo do Tessarolo. Quem divulgar manda o público para a
      live do dia anterior. → **Leo**
- [ ] 🔴 **Revisar a escala de Libras** — a Eliana está nos 7 momentos, sem folga. → **Carol / Eliana**
- [ ] 🔴 **Definir o plano B do app** — os servidores do Ifes estão fora do ar e não se sabe se ele
      sobe na segunda. O que a equipe fala no palco e no credenciamento se não subir?
      → **Marquito / Elton**
- [ ] 🟡 **Completar os thumbnails que faltam** — 6 das 8 lives já têm capa *(feitas hoje de manhã,
      em [`thumbs-youtube/`](../producao/02-pre-evento/thumbs-youtube/))*. **Faltam o do Educimat
      (2 blocos, 20/08) e o da premiação (20/08)**, que dependem de confirmar o que a organização
      quer transmitir na quinta. ✅ Já existem também 3 genéricos: capa, "iniciaremos em instantes"
      e em branco.
- [ ] 🟡 **Programar a contagem regressiva hoje ou amanhã** — 15 e 16/08 são **sábado e domingo**,
      exatamente o que derrubou o feed em 01–02/08. ✅ O card *"últimos dias para se inscrever"*
      já está produzido. → **Elton**
- [ ] 🟡 **Decidir as artes dos 2 cards de mesa** ("Desafios da EaD" e "Tecnologia Delas"). Se for
      fazer, é agora: restam 3 dias de feed e a regressiva ocupa os mesmos dias. → **Elton**

---

## 📦 Amanhã, 14/08 (sexta) — o dia da gráfica

- [ ] 🔴 **Conferir todas as peças impressas quando chegarem** — brindes, 10 banners do TR e os 3
      institucionais, tudo no mesmo lote. É a última chance de pegar erro de horário, nome ou
      sala. → **Elton**
- [ ] 🟡 **Reforçar com a gráfica que a entrega venha também por e-mail**, não só por WhatsApp —
      pedido feito em 28/07 e nunca atendido.
- [ ] 🟡 **Conferir a grade das camisas na entrega:** PP 2 · P 20 · M 30 · G 28 · GG 16 · XG 2 ·
      XXG 2 = 100.

---

## 📅 Fim de semana e véspera (15–17/08)

### Publicações
- [ ] 🔴 **Card de 15/08** — faltam 2 dias + último dia de inscrição *(sábado: programar antes)*
- [ ] 🔴 **Card de 16/08** — é amanhã *(domingo: programar antes)*
- [ ] 🔴 **Card de 17/08** — é hoje
- [ ] 🔴 **Notícia do e-book do NTE no site** — texto, HTML para o Joomla e ficha técnica prontos
      em [`noticia-site.md`](../producao/02-pre-evento/livro-nte/noticia-site.md). **Só falta o
      link** para publicar.
- [ ] 🟡 **Card do e-book do NTE** — ✅ sinopse resolvida pelo PDF oficial; a capa do e-book serve
      de imagem. 🔴 Faltam **o link** e **quem faz a arte**. **⚠️ arte sem dono**
- [ ] 🔵 **Divulgação dos 5 lançamentos** (livro 20 anos · Base de Conhecimentos · MOOCs · e-book
      do NTE · IA no Moodle) — pode ir em carrossel com chamada de inscrição
- [ ] 🟡 **Notícia para a ACS** — rascunho pronto, falta a **revisão da Márcia**

### Preparação da operação
- [ ] 🔴 **Modelo único de slides** — pedido da Coordenação, para palestrantes, sessões técnicas,
      lançamentos e premiação. **⚠️ sem dono**
- [ ] 🔴 **Termos de autorização de imagem e voz — só para os podcasts.** Não é necessário para
      transmissão, fotografia nem cobertura: vale apenas para quem senta na cadeira do podcast.
      Obter/adaptar o modelo de 2024, imprimir as cópias e definir onde ficam guardados.
      Sem termo assinado, não grava. → **Andreia** *(já é a responsável nos 4 dias)*
- [ ] 🟡 **Criar o espaço compartilhado de upload** do material gravado em celular e divulgar o
      link para a equipe. → **Elton** *(responsável pelas postagens)*
- [ ] 🟡 **Fechar a aba de links úteis da planilha** — falta o link do app (campo vazio) e o da
      programação. → **Juliana**
- [ ] 🟡 **Alinhar o que entra e o que não entra nos stories**, incluindo política de repost, para
      a cobertura não virar excesso.
- [ ] 🟡 **Definir quem recebe os palestrantes** — Vanessa (17/08) e Tessarolo (18/08) seguem
      `___` na planilha. **⚠️ sem dono**
- [ ] 🟡 **Definir o apoio da quinta-feira** — Marquito e Elton ministram oficina nos dois turnos e
      ficam fora da cobertura, no dia com **6 eventos paralelos simultâneos**. **⚠️ sem dono**
- [ ] 🔵 **Definir horário e ponto de encontro da foto oficial** dos servidores e colaboradores no
      encerramento (em 2024 foi no palco).
- [ ] 🔵 **Avaliar as reações do app sobre a transmissão ao vivo** — pedido do Marquito (overlay
      transparente, sem fundo nem título). Decidir se faz sentido.
- [ ] 🔵 **Elton: decidir entre a aula de segunda à noite e a abertura** — a aula começa 18h20 e é
      a noite da abertura, dos lançamentos e da celebração dos 20 anos.

---

## ☎️ Confirmações com a Coordenação / comissão

- [ ] 🔴 **Escopo da cobertura** — que a CGTE faz registro **geral**, não apresentação por
      apresentação nas sessões técnicas e na mostra. Confirmar para não virar cobrança depois.
      → **Marquito leva à Márcia**
- [ ] 🔴 **O que transmitir na quinta (20/08)** — a organização pediu transmissão do aniversário de
      15 anos do Educimat, mas não mandou título nem disse se é o dia todo. **Trava os 2
      thumbnails que faltam.** → **Marquito**
- [ ] 🟡 **Confirmar a permanência de Mariano e Mauro até quinta**, se a grade de podcasts que
      vencer depender do dia 20.
- [ ] 🟡 **Chat do YouTube desativado por legislação eleitoral** — confirmar se é orientação formal
      do Ifes ou decisão nossa.
- [ ] 🟡 **Logo do Educimat** para os impressos.
- [ ] 🟡 **Atualização do Even3 para espelhar o site** — o app lê de lá.
- [ ] 🔵 **Materiais dos palestrantes para o app** — a Márcia solicitou e não houve retorno.

---

## 🎬 Durante o evento (17 a 20/08)

> A escala nominal está na
> [planilha](https://bit.ly/conceforcgte).
> Responsáveis fixos: postagens **Elton** · app admin **Marquito** · termos **Andreia** ·
> transmissão **Leo e Tiago** · cobertura **Nanda, Andreia e Juliana**.

- [ ] **17/08 13h30** — montagem dos equipamentos *(Tiago, com Marquito, Carol e Elton)*
- [ ] **17/08** — cobertura do credenciamento · momento musical · abertura · palestra da Vanessa ·
      4 lançamentos · ⭐ **celebração do bolo dos 20 anos**
- [ ] **18/08** — mesa "Tecnologia Delas" · palestra Tessarolo · mostra de produtos ·
      sessões técnicas · **2 podcasts**
- [ ] **19/08** — momento cultural · mesa "Desafios da EaD" · palestra Mariano · palestra Mauro ·
      sessões técnicas · **2 podcasts**
- [ ] **20/08** — 6 eventos paralelos simultâneos *(cobertura em rodízio entre as salas)* ·
      ⭐ **premiação e foto dos vencedores** · momento cultural · ⭐ **foto oficial dos servidores**
- [ ] **Diário** — stories da programação do dia + reel de fechamento do dia
- [ ] **Operar as 8 transmissões** com Libras
- [ ] **Colher os termos assinados antes de cada gravação de podcast** — só os podcasts pedem
      termo. → **Andreia**

---

## 🗂️ Pós-evento (a partir de 21/08)

- [ ] 🟡 **Retrospectiva, números e agradecimentos** **⚠️ sem dono**
- [ ] 🟡 **Divulgação dos trabalhos premiados** **⚠️ sem dono**
- [ ] 🟡 **Edição e publicação dos podcasts** — definir responsável, canal e data **⚠️ sem dono**
- [ ] 🔵 **Publicação das gravações das transmissões** **⚠️ sem dono**
- [ ] 🔵 **Relatório final** — o app gera o dashboard; definir quem escreve a leitura

---

## ✅ Já resolvido (não fazer nada)

| Frente | Como ficou |
|---|---|
| Etiquetas de nome do crachá | **comissão geral** assumiu a impressão dos nomes |
| Cantinho instagramável | **Luciane Lacerda e Carol (CGA)** |
| Banners institucionais | **na gráfica**, prontos até sexta |
| Link do livro dos 20 anos | **definitivo**, no site institucional |
| Vídeos-convite dos palestrantes | **cancelados** (e os reels que dependiam deles) |
| Kit dos eventos paralelos | **enviado e publicado** em 11/08 |
| Sinopse do e-book do NTE | **resolvida pelo PDF oficial** (13/08) |
| Notícia do e-book para o site | **escrita**, só falta o link |
| Thumbnails das lives | **6 das 8 prontas** (13/08) + 3 genéricos |
| Card "últimos dias para se inscrever" | **produzido** (13/08) |
| Banner #13 · vitrine de MOOCs | **arte atualizada** (13/08) |
| **Fundo de palco** | ❌ **não vai ter** (13/08) — encerra a dúvida do TR |
| **Arte do bolo dos 20 anos** | ✅ **pronta, a Andreia fez tudo** (13/08) |

---

## O resumo do resumo

**49 itens em aberto**, dos quais **8 seguem sem dono** (marcados com ⚠️): a arte do card do NTE,
o modelo de slides, quem recebe os palestrantes, o apoio da quinta e as **4 frentes de
pós-evento**. São eles que produzem o risco real, porque tarefa sem nome não acontece sozinha —
e metade está no pós-evento, que é quando a equipe já se dispersou.

**Os 3 que não podem escorregar:**
1. **Link de hospedagem do e-book do NTE** — trava a notícia, o card e o QR, e o lançamento é
   segunda às 20h. É a última peça de uma frente que já está 90% pronta.
2. **Grade dos podcasts** — a reunião é hoje, e há duas versões conflitantes em circulação.
3. **Plano B do app** — os servidores do Ifes estão fora do ar.
